...
def run_windows_program(args, add_env=None, cwd=None):
    ...
    if sys.platform == "win32":
        ...
        subprocess.check_call([str(SCRIPTS_DIR / "wineth08")] + args, env=env, cwd=cwd)
...
def run_windows_program_output(args, add_env=None, cwd=None) -> str:
    ...
    if sys.platform == "win32":
        ...
        return subprocess.check_output(
            [str(SCRIPTS_DIR / "wineth08")] + args, env=env, cwd=cwd
        )
...
def get_windows_path(path):
    if sys.platform == "win32":
        ...
        return subprocess.check_output(
            [str(SCRIPTS_DIR / "wineth08"), "winepath", "-w", str(path)], text=True
        ).strip()
